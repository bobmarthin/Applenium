...
using System.Linq;
using Constants = Applenium.Constants;
...
namespace Applenium
{
    ...
    public partial class MainWindow
    {
        ...
        /// <summary>
        ///     This is main window of Applenium
        /// </summary>
        public MainWindow()
        {
            try
            {
                ...
                _driver = sel.SetWebDriverBrowser(_driver, browser, true);
            }
            ...
            try
            {
                ...
                switch (tableName)
                {
                    ...
                    case Constants.StrGuiProjectsNewTest:
                        ...
                        break;
                    case
                        Constants.StrGuiProjectsNewScenario:
                        //dataset.Clear();
                        //dataset.EnforceConstraints = false;
                        adapterScenario.Fill(dataset.Scenario);
                        adapterProjectsTest.Fill(dataset.Projects);
                        ...
                        break;
                    case
                        Constants.StrGuiProjectsNewBatch
                        :
        ...
        private void DataGridGuiMap_RowEditing_Edit(object sender, DataGridRowEditEndingEventArgs e)
        {
            try
            {
                ...
                var adapterTest = new GuiMapTableAdapter();
                ...
                if (rowView != null)
                {
                    ...
                    if (guiMapId == -1)
                    {
                        adapterTest.Insert(guiMapObjectName, tagTypeId, tagTypeValue, pageSectionId);
                        DataGridFill(Constants.StrGuiMapEdit, "0");
                    }
                    else
                    {
                        adapterTest.UpdateBy(guiMapObjectName, tagTypeId, tagTypeValue, pageSectionId, guiMapId);
                        DataGridFill(Constants.StrGuiMapEdit, _guimapId.ToString(CultureInfo.InvariantCulture));
                    }
                }
            }
            catch (Exception exception)
            ...
            try
            {
                ...
                if (e.Key == Key.Delete)
                {
                    if (_editinggrid == false)
                    {
                        ...
                        if (
                            MessageBox.Show("Delete " + guiMapObjectName + "?", "Applenium",
                                            MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                            adapterTest.Delete(guiMapId, guiMapObjectName, tagTypeId, tagTypeValue, guiProjectId);
                    }
                }
                if (e.Key == Key.Enter)
                {
        ...
        private void dataGridGuiMap_PreviewKeyDown_Edit(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.Key == Key.Delete)
                {
                    ...
                    using (var adapterTest = new GuiMapTableAdapter())
                    {
                        if (_editinggrid == false)
                        {
                            int guiMapId = Convert.ToInt32(rowView.Row["GuiMapID"].ToString());
        ...
        private void DataGridTestEditor_RowEditing(object sender, DataGridRowEditEndingEventArgs e)
        {
            ...
            try
            {
                ...
                using (var adapterTest = new GuiTestStepsTableAdapter())
                {
                    ...
                    if (rowView != null)
                    {
                        ...
                        {
                            rowView.Row["IsValidate"] = 0;
                            stepsOrder = Convert.ToInt32(adapterTest.LastStepsOrder(guiTestId)) + 1;

                            adapterTest.Insert(guiTestId, guiMapId, guiMapCommandId, inputDataColumn, isValidate,
                                               stepsOrder);
                        }
                        else
                        {
                            stepsOrder = Convert.ToInt32(rowView.Row["StepsOrder"].ToString());
                            adapterTest.UpdateBy(guiTestId, guiMapId, guiMapCommandId, inputDataRow, inputDataColumn,
                                                 isValidate, stepsOrder, guiTestStepsId);
                        }
                    }
                }
            }
                    ...
                    ThreadStart ts = delegate
                        {
                            ...
                            {
                                result = ExecuteTest(selectedItem, rowNumber, runExecutionId, _driver);
                            }
        ...
        private bool ExecuteTest(string selectedItem, string rowNumber, int runExecutionId, IWebDriver driver)
        {
            var exman = new ExecutionManager(runExecutionId);
            bool result = exman.ExecuteOneTest(selectedItem, rowNumber, driver);
                ...
                if (e.Key == Key.Delete)
                {
                    ...
                    using (var adapterTest = new GuiTestStepsTableAdapter())
                    {
                        if (_editinggrid == false)
                        {
                            ...
                            if (MessageBox.Show("Delete Selected Step?", "Applenium", MessageBoxButton.YesNo) ==
                                MessageBoxResult.Yes)
                            {
                                int currentStepsOrder = Convert.ToInt32(rowView.Row["StepsOrder"].ToString());
                                int selectItem = currentStepsOrder - 1;
                                while (adapterTest.GetStepsId(guiTestId, currentStepsOrder + 1) != null)
                                {
                                    int nextStepsOrder = currentStepsOrder + 1;
                                    int nextGuiTestStepsId =
                                        Convert.ToInt32(adapterTest.GetStepsId(guiTestId, nextStepsOrder));
                                    adapterTest.UpdateStepsOrder(currentStepsOrder, nextGuiTestStepsId);
        ...
        private void MenuItem_AddColumn_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (columname != string.Empty)
                {
                    ...
                    sql.AddColumn(tableName, columname);
                    DataGridFill(Constants.StrInputTable, _testid.ToString(CultureInfo.InvariantCulture));
                    DataGridFill(Constants.StrGuiTestSteps, _testid.ToString(CultureInfo.InvariantCulture));
                }
            }
        ...
        private bool ExecuteScenario(string selectedItem, int runExecutionId, IWebDriver driver)
        {
            var exman = new ExecutionManager(runExecutionId);
            ...
            try
            {
                if (e.Key == Key.Delete)
                {
                    ...
                    if (_editinggrid == false)
                    {
                        ...
                        if (MessageBox.Show("Delete Selected Test? ", "Applenium", MessageBoxButton.YesNo) ==
                            MessageBoxResult.Yes)
                        {
                            int currentStepsOrder = Convert.ToInt32(rowView.Row["StepsOrder"].ToString());
                            int selectItem = currentStepsOrder - 1;
                            while (adapterScenario.GetStepsId(guiScenarioId, currentStepsOrder + 1) != null)
                            {
                                int nextStepsOrder = currentStepsOrder + 1;
                                int nextGuiTestStepsId =
                                    Convert.ToInt32(adapterScenario.GetStepsId(guiScenarioId, nextStepsOrder));
            ...
            try
            {
                ...
                if (rowView != null)
                {
                    ...
                    if (rowView.Row["ExecutionStatusID"].ToString() == string.Empty)
                        executionStatusId = Constants.NotRunnig;
                    else
                ...
                if (e.Key == Key.Delete)
                {
                    ...
                    using (var adapterScenario = new BatchLogicTableAdapter())
                    {
                        if (_editinggrid == false)
                        {
                            int guiBatchLogicId = Convert.ToInt32(rowView.Row["BatchLogicID"].ToString());
                            if (
                                MessageBox.Show("Delete Selected Scenario? ", "Applenium", MessageBoxButton.YesNo) ==
                                MessageBoxResult.Yes)
        ...
        private void WindowApplenium_Closed(object sender, EventArgs e)
        {
            try
            {
                _driver.Quit();
        ...
        private void btnDeleteObject_Click(object sender, RoutedEventArgs e)
        {
            ...
            MessageBox.Show("Select row (object) and press delete button on keyboard", "Applenium");
        }
        ...
        {
            MessageBox.Show("You can't delete test plaese ask Automation Manager", "Applenium");
        }
        ...
        {
            MessageBox.Show("You can't delete Scenario please ask Automation Manager", "Applenium");
        }
        ...
        {
            MessageBox.Show("You can't delete Batch please ask Automation Manager", "Applenium");
        }
        ...
        private void MenuItem_AddNewPage_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                adapterProjectpage.Insert1(_projectid, pageName);
                DataGridFill(Constants.StrGuiProjects, null);
                DataGridFill(Constants.StrGuiTest, null);
            }
        }
        ...
        private void MenuItem_AddNewPageSection_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                adapterPagesection.Insert(_projectPageid, pageSectionName);
                DataGridFill(Constants.StrGuiProjects, null);
                DataGridFill(Constants.StrGuiTest, null);
            }
        }
        ...
        private void MenuItem_RenamePage_Click(object sender, RoutedEventArgs e)
        {
            if (_projectPageid != 0)
            {
                ...
                if (pageName != string.Empty)
                {
                    adapterProjectpage.UpdateBy(pageName, _projectPageid);
                    DataGridFill(Constants.StrGuiProjects, null);
                    DataGridFill(Constants.StrGuiTest, null);
                }
            }
        ...
        private void MenuItem_RenamePageSection_Click(object sender, RoutedEventArgs e)
        {
            if (_pageSectionid != 0)
            {
                ...
                if (pageName != string.Empty)
                {
                    adapterPageSection.Update(pageName, _pageSectionid);
                    DataGridFill(Constants.StrGuiProjects, null);
                    DataGridFill(Constants.StrGuiTest, null);
                }
            }
        ...
        private void WindowApplenium_Closing(object sender, CancelEventArgs e)
        {
            _driver.Quit();
        }

        private void WindowApplenium_Unloaded(object sender, RoutedEventArgs e)
        {
            _driver.Quit();
        }
        ...
        private void DataGrid_Configuration_RowEditEnding(object sender, DataGridRowEditEndingEventArgs e)
        {
            ...
            if (rowView != null)
            {
                ...
                if (rowView["Name"].ToString() == "DefaultBrowser")
                    ...
                    _driver = sel.SetWebDriverBrowser(_driver, value, true);
            }
        }
        ...
        {
            int selectedItem = 0;
            var adapter = new GuiScenarioLogicTableAdapter();
            ...
            try
            {
                ...
                if (rowView != null)
                {
                    ...
                    int currentStepsOrder = Convert.ToInt32(rowView.Row["StepsOrder"].ToString());
                    int currentguiTestStepsId = Convert.ToInt32(adapter.GetStepsId(scenarioOrTestId, currentStepsOrder));
                    int previesStepsOrder = currentStepsOrder - 1;
                    if (adapter.GetStepsId(scenarioOrTestId, previesStepsOrder) != null)
                    {
                        int previesGuiTestStepsId =
                            Convert.ToInt32(adapter.GetStepsId(scenarioOrTestId, previesStepsOrder));
                        adapter.UpdateStepsOrder(previesStepsOrder, currentguiTestStepsId);
                        adapter.UpdateStepsOrder(currentStepsOrder, previesGuiTestStepsId);
                        selectedItem = previesStepsOrder - 1;
                    }
                    if (currentStepsOrder == 1)
                    {
                        currentguiTestStepsId = Convert.ToInt32(adapter.GetStepsId(scenarioOrTestId, currentStepsOrder));
                        while (adapter.GetStepsId(scenarioOrTestId, currentStepsOrder + 1) != null)
                        {
                            int nextStepsOrder = currentStepsOrder + 1;
                            int nextGuiTestStepsId =
                                Convert.ToInt32(adapter.GetStepsId(scenarioOrTestId, nextStepsOrder));
                            adapter.UpdateStepsOrder(currentStepsOrder, nextGuiTestStepsId);
                            currentStepsOrder = nextStepsOrder;
                        }
                        adapter.UpdateStepsOrder(datagrid.Items.Count - 1, currentguiTestStepsId);
        ...
        private void MoveStepDownScenario(DataGrid datagrid, string dataGridEditor)
        {
            ...
            try
            {
                ...
                using (var adapter = new GuiScenarioLogicTableAdapter())
                {
                    if (rowView != null)
                    {
                        ...
                        int currentguiTestStepsId =
                            Convert.ToInt32(adapter.GetStepsId(scenarioOrTestId, currentStepsOrder));
                        int nextStepsOrder = currentStepsOrder + 1;
                        int nextGuiTestStepsId;
                        if (adapter.GetStepsId(scenarioOrTestId, nextStepsOrder) != null)
                        {
                            nextGuiTestStepsId = Convert.ToInt32(adapter.GetStepsId(scenarioOrTestId, nextStepsOrder));
                            adapter.UpdateStepsOrder(nextStepsOrder, currentguiTestStepsId);
                            adapter.UpdateStepsOrder(currentStepsOrder, nextGuiTestStepsId);
                        ...
                        if (currentStepsOrder == datagrid.Items.Count - 1)
                        {
                            ...
                            adapter.UpdateStepsOrder(1, currentguiTestStepsId);
                            selectedItem = 0;
                        }
                    }
                    DataGridFill(dataGridEditor, scenarioOrTestId.ToString(CultureInfo.InvariantCulture));
        ...
        {
            int selectedItem = 0;
            var adapter = new GuiTestStepsTableAdapter();
        ...
        private void MoveStepDownTest(DataGrid datagrid, string dataGridEditor)
        {
            ...
            try
            {
                ...
                using (var adapter = new GuiTestStepsTableAdapter())
                {
                    if (rowView != null)
                    {
                        //int currentguiTestStepsId = Convert.ToInt32(rowView.Row["GuiTestStepsID"].ToString());
        ...
        private void CopyTest(object sender, RoutedEventArgs e)
        {
            ...
            adapterTest.CopyTest(newtestname, Constants.UncompletedTestImage, _testid);
            int? newtestid = adapterTest.GetLastTestId();
        ...
        private void ButtonSwitchWindow_Click(object sender, RoutedEventArgs e)
        {
            ...
            string currentTitle = _driver.Title;



            WindowApplenium.Title = "Applenium. Current window - " + currentTitle;
